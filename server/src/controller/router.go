...
import (
	...
	"github.com/ssizawa/go_mysql/server/src/repository"
	"github.com/ssizawa/go_mysql/server/src/structure"
)
...
func Router(router *gin.Engine) {
	...
	router.POST("/add_user", func(c *gin.Context) {

		user_name := c.PostForm("name")
		password := c.PostForm("password")

		repository.Adduser(user_name, password)
		session := sessions.Default(c)
		session.Set("user_name", user_name)
		session.Save()
